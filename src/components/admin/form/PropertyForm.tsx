...
const EQUIPMENT_LIST = [
  "Climatisation", "Wi-Fi", "Parking", "Piscine", 
  "Jardin", "Groupe électrogène", "Gardiennage 24h", 
  "Cuisine équipée", "Terrasse", "Balcon", 
  "Titre foncier", "Sécurité"
]
...
export default function PropertyForm({ id }: PropertyFormProps) {
  ...
  const addPhoto = () => {
    if (!newPhotoUrl.startsWith("http")) {
      toast({ title: "Erreur", description: "L'URL doit commencer par http:// ou https://", variant: "destructive" })
      return
    }
    setFormData(prev => ({
      ...prev,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.title || formData.title.length < 5) {
      toast({ title: "Erreur", description: "Le titre doit faire au moins 5 caractères", variant: "destructive" })
      setActiveTab(0)
      return
    }
    if (!formData.description || formData.description.length < 50) {
      toast({ title: "Erreur", description: "La description doit faire au moins 50 caractères", variant: "destructive" })
      setActiveTab(0)
      return
    }
    ...
    return (
      <div className="bg-white rounded-2xl p-6 shadow-sm border border-border animate-pulse">
        <div className="flex gap-4 mb-8 border-b pb-4">
          {[1,2,3,4].map(i => <div key={i} className="h-10 w-32 bg-muted rounded-lg"></div>)}
        </div>
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-sm border border-border flex flex-col relative overflow-hidden">
      {/* Tabs Header */}
        ...
        {activeTab === 0 && (
          <div className="space-y-6 max-w-3xl animate-in fade-in duration-300">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                <label className="text-sm font-medium block">Type d'offre *</label>
              ...
              <div className="space-y-2 flex flex-col justify-center pt-6">
                <label className="flex items-center gap-3 cursor-pointer">
                  <div className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${formData.isFeatured ? 'bg-[#F39C12]' : 'bg-muted-foreground/30'}`}>
                    <input 
                      type="checkbox" 
                      className="sr-only" 
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                ...
                <input
                  ...
                  required
                />
                <p className="text-xs text-muted-foreground">Location = mensuel / Vente = total</p>
              </div>
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  onChange={e => {
                    ...
                    handleChange("location", e.target.value) // Sync location field as backup
                  }}
                  className="w-full px-4 py-2.5 rounded-xl border border-input focus:ring-2 focus:ring-[#1A5276] outline-none transition-all"
        ...
        {activeTab === 2 && (
          <div className="space-y-8 max-w-3xl animate-in fade-in duration-300">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="space-y-2 flex flex-col justify-center pt-6">
                <label className="flex items-center gap-3 cursor-pointer">
                  <div className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${formData.isFurnished ? 'bg-[#1E8449]' : 'bg-muted-foreground/30'}`}>
                    <input 
                      type="checkbox" 
                      className="sr-only" 
                    ...
                    />
                    <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${formData.isFurnished ? 'translate-x-6' : 'translate-x-1'}`} />
                  </div>
            ...
            <div className="space-y-4 border-t border-border pt-6">
              <label className="text-base font-semibold">Équipements et options</label>
        ...
        {activeTab === 3 && (
          <div className="space-y-8 animate-in fade-in duration-300">
            <div className="max-w-xl space-y-4">
              <label className="text-sm font-medium">Ajouter une photo (URL)</label>
            ...
            {formData.images && formData.images.length > 0 ? (
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                {formData.images.map((img, idx) => (
                  <div key={idx} className="relative group rounded-xl overflow-hidden border border-border aspect-square bg-muted">
                    <img 
                      src={img} 
                      alt={`Photo ${idx + 1}`} 
              ...
              <div className="border-2 border-dashed border-border rounded-2xl p-12 flex flex-col items-center justify-center text-muted-foreground">
                ...
                <p className="text-sm mt-1">La première photo sera utilisée comme image principale.</p>
              </div>
            )}
          </div>
        )}
